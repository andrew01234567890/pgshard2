...
// An open range extends to the top of the space (2^64).
...
var FullRange = KeyRange{open: true}
...
func NewOpenKeyRange(start uint64) KeyRange {
	return KeyRange{start: start, open: true}
}

func (r KeyRange) Start() uint64 { return r.start }

// End returns the exclusive end bound; ok is false for an open range.
func (r KeyRange) End() (end uint64, ok bool) { return r.end, !r.open }

func (r KeyRange) IsFull() bool { return r.open && r.start == 0 }

func (r KeyRange) Contains(id KeyspaceID) bool {
	return uint64(id) >= r.start && (r.open || uint64(id) < r.end)
}

func (r KeyRange) Intersects(other KeyRange) bool {
	return (other.open || r.start < other.end) && (r.open || other.start < r.end)
}
