...
fn plan(spec: &v1::WorkflowSpec, config: &WorkflowConfig) -> Result<RunPlan, WorkflowError> {
    ...
    let (range, hash) = match spec.filter.as_ref().and_then(|f| f.filter.as_ref()) {
        ...
        Some(v1::row_filter::Filter::KeyRange(kr)) => {
            let range = kr
                .range
                .as_ref()
                .map(|r| KeyRange::new(r.start, r.end.unwrap_or(0)))
    ...
    for mapping in &spec.tables {
        ...
        if let Some(target) = &mapping.target {
            if (target.schema != src.schema || target.name != src.name)
...
async fn run_workflow(
    ...
    let snapshot = repl
        .create_logical_slot_exported(&run.slot)
        .await
    ...
    target_sql
        ...
        .ok();
    let _ = target_sql
        .execute("DELETE FROM pgshard.repl_progress WHERE consumer = $1", &[&run.id])
        .await; // table may not exist yet; the applier creates it
    ...
    let mut applier = Applier::new(connect_sql(&config.target, &run.target_database).await?, &*run.id)
        .await
    ...
    loop {
        ...
        match &msg {
            ...
            LogicalRepMsg::Insert(ins) => {
                let f = known(&relations, ins.rel_oid)?;
                if tuple_in_range(&ins.new_tuple, f.key_index, f.key_type, shard_fn, &run.range)? {
                    applier.handle(&msg).await?;
                }
            }
            LogicalRepMsg::Update(upd) => {
                // The shard key is immutable, so the new tuple decides.
                let f = known(&relations, upd.rel_oid)?;
                if tuple_in_range(&upd.new_tuple, f.key_index, f.key_type, shard_fn, &run.range)? {
                    applier.handle(&msg).await?;
                }
            }
            LogicalRepMsg::Delete(del) => {
                let f = known(&relations, del.rel_oid)?;
                let tuple = del
                    .key
                    .as_ref()
                    .or(del.old.as_ref())
        ...
        if committed {
            ...
            status.send_modify(|s| {
                s.applied_lsn = Some(v1::Lsn { value: ack.into() });
            });
        }
    }
}
...
fn known<'a>(
    relations: &'a HashMap<u32, impl Sized>,
    oid: u32,
) -> anyhow::Result<&'a impl Sized> {
    relations
        .get(&oid)
